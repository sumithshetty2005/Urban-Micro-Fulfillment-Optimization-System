...
def solve_routing(
    ...
    for i in range(num_nodes):
        for j in range(num_nodes):
            if i == j:
                ...
                distance_matrix[i][j] = int(np.round(dist_km * 1000.0))  # Convert to meters
                
    # Prepare package demands scaled to integer values (weight in KG * 100)
    demands = [0]  # Depot demand is zero
    for _, row in df_del.iterrows():
    ...
    dist_penalty_per_km = 8.33  # Fixed wear & tear / distance penalty per km
    
    # Set objective cost coefficient based on optimization goal
    if optimization_goal == "Lowest Fuel Cost":
        # Minimize ONLY fuel cost
        cost_per_meter_paise = (fuel_cost_per_km * 100) / 1000.0
        fixed_salary_paise = 0  # no driver penalty
    ...
    # Register time callback (includes travel time + 5 mins service time per stop)
    ...
    routing.AddDimension(
        ...
        100000,  # max time limit (10000 mins)
        True,  # start cumulative time to zero
        'Time'
    )
    time_dimension = routing.GetDimensionOrDie('Time')
    
    # Handle specific optimization goals
    if optimization_goal == "Fastest Delivery":
        time_dimension.SetGlobalSpanCostCoefficient(100)
        
    # Add soft upper limits for delivery time constraint (scaled by 10)
    ...
    # Register weight demand callback
    ...
    routing.AddDimensionWithVehicleCapacity(
        ...
        True,  # start cumulative load to zero
        'Capacity'
    )
    ...
    for vehicle_id in range(fleet_size):
        ...
        # Calculate late delivery penalty: average speed 40km/h (1.5 min per km) + 5 mins service time per stop
        ...
        if total_time_mins > max_delivery_time:
            ...
            late_delivery_penalty = overtime_mins * 5.0  # ₹5 per minute
            
        # Format the stops sequence for the vehicle
        stops = []
        ...
        # Double the late penalty if route contains any priority order
